...
This project implements a custom, high-performance distributed deep learning framework from scratch. It builds neural network primitives (Linear, CNN, BatchNorm, Dropout) and optimizers (SGD, AdamW) without relying on PyTorch or TensorFlow for the core logic.
...
Key Features
    ...
    Mixed Precision Training: Implements FP16 storage for weights/gradients with FP32 master weights and dynamic loss scaling to maintain numerical stability during training.
...
Use this to test GPU acceleration and synchronization on a simpler dataset (MNIST) without the complexity of CNN layers. This uses allreduce_worker_gpu.py.

Command Syntax:
Bash

python3 allreduce_worker_gpu.py <rank> <world_size> <batch_size> <hidden_size> <learning_rate>
...
Use this for basic debugging of the ring communication logic if no GPU is available. This uses allreduce_worker.py.
...
(Note: Hyperparameters like batch size and learning rate are hardcoded in this script)
